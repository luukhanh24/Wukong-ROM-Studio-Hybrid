...
class TelegramMiniAppTests(unittest.TestCase):
    ...
    def test_mobile_surface_is_distilled_and_maintenance_is_admin_only(self) -> None:
        ...
        self.assertIn('input:not([type="checkbox"]):not([type="radio"]), select, textarea { font-size:16px !important; }', styles)
        self.assertNotIn('user-scalable=no', html)
        self.assertNotIn('maximum-scale=1', html)
        self.assertIn('bottom:calc(104px + var(--safe-bottom))', styles)
        self.assertIn('.bottom-nav button:not(.dock-profile) { top:4px; height:56px; min-height:56px;', styles)
