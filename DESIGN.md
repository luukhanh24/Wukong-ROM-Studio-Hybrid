...
Desktop uses a sticky four-cell contents rail with consistent line icons. Mobile adapts Kyant0's LiquidBottomTabs material model as a five-slot web control: a single refractive glass shell forms a wide capsule with a compact round center rise fitted closely around the avatar. The center cap stays 76px wide around the 56–60px avatar instead of stretching with the viewport; the shell outline and glass share that same silhouette at every mobile width. No separate ring or arc surrounds the avatar. The top sheen is omitted so page content remains visible through the dock; labels and icons use near-black in light mode and white in dark mode. A distinct 18px refractive lens retains subtle rim and chromatic edge separation while settling with a smooth exponential ease-out without bounce. The Profile destination suppresses the lens and may use only a soft avatar-derived halo, never a circular rim. Reduced-motion preference removes lens travel, halo scale and rotating greeting transitions.
