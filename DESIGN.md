...
Desktop uses a sticky four-cell contents rail with consistent line icons. Mobile adapts Kyant0's LiquidBottomTabs material model as a five-slot web control: a single refractive glass shell forms a wide capsule with a compact round center rise fitted closely around the avatar. The WebView-safe CSS mask keeps the center cap 76px wide around the 56–60px avatar instead of stretching with the viewport or relying on an SVG compositing filter. No separate ring or arc surrounds the avatar. The shell and lens use 22px backdrop blur, strong vibrancy and low-alpha tint so underlying content remains visible but optically softened; labels and icons use near-black in light mode and white in dark mode. The lens retains subtle rim, inner depth and chromatic edge separation while settling with a smooth exponential ease-out without bounce. The Profile destination suppresses the lens and may use only a soft avatar-derived halo, never a circular rim. Reduced-motion preference removes lens travel, halo scale and rotating greeting transitions.
