...
LOCAL_FASTBOOT="$SCRIPT_DIR/bin/linux/fastboot"
LOCAL_ADB="$SCRIPT_DIR/bin/linux/adb"
mapfile -t lines < "$SCRIPT_DIR/info.txt"
...
if [ -f "$LOCAL_FASTBOOT" ]; then
    ...
    echo "Error: fastboot not found in bin/linux/ or system PATH."
    exit 1
fi
...
if [ -f "$LOCAL_ADB" ]; then
    ...
    echo "Error: adb not found in bin/linux/ or system PATH."
    exit 1
fi
...
while IFS= read -r line || [ -n "$line" ]; do
    ...
    if [ -z "$found_path" ]; then
        ...
        current_md5=$(md5sum "$found_path" | awk '{print $1}')
