...
class TelegramMiniAppTests(unittest.TestCase):
    ...
    def test_mobile_surface_is_distilled_and_maintenance_is_admin_only(self) -> None:
        ...
        self.assertIn('.bottom-nav button:not(.dock-profile) { top:3px; height:56px; min-height:56px;', styles)
        self.assertIn("const bodyTop = 24;", script)
        self.assertIn("const capCenterY = 45;", script)
        self.assertIn("const capShoulder = Math.sqrt", script)
        self.assertIn("`A ${capRadius} ${capRadius} 0 0 1", script)
