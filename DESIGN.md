...
The form language mixes precise controls with selectively softer instruments. Fields and compact controls use four- to ten-pixel corners. Grouped MODs and source instruments use 12–14px corners. Major sheets use 16–18px corners. The mobile navigation is a deliberate floating instrument with a 31px-radius moving glass lens; the compact dispatch shortcut may use a capsule because it is a transient edge action, not a global control convention.
...
Desktop uses a sticky four-cell contents rail with consistent line icons. Mobile combines Kyant0's LiquidBottomTabs motion relationships with a KernelSU-like neutral frosted material: a low-tint shell uses 20px bounded backdrop blur and vibrancy, while each work destination receives a centered 62px circular glass lens with highlight, inner depth and restrained chromatic edge separation. Pressing grows and velocity-deforms the lens independently on each axis while the whole panel receives a damped four-pixel offset and a subtle 16px-relative scale. Release settles with exponential ease-out and no bounce. The shell follows the user-drawn silhouette: a rounded horizontal capsule with a smooth centered dome, generated at the measured mobile width so the 56–60px avatar remains closely fitted without stretching the cap. Dark mode uses a transparent midnight tint instead of a neutral gray fill. No separate ring or arc surrounds the avatar. Profile suppresses the moving lens and may use only a soft avatar-derived halo. Labels and icons use near-black in light mode and white in dark mode. Reduced-motion preference removes lens travel, deformation, halo scale and rotating greeting transitions.
