...
Desktop uses a sticky four-cell contents rail with consistent line icons. Mobile ports Kyant0's LiquidBottomTabs material relationships into a five-slot web control: a 40%-tinted container with 8px backdrop blur and vibrancy, plus a separate refractive tab lens with highlight, shadow, inner depth and chromatic edge separation. Pressing grows the 56px lens toward 78px; drag velocity deforms its horizontal and vertical scale independently while the whole panel receives a damped four-pixel offset and a subtle 16px-relative scale. Release settles with exponential ease-out and no bounce. The shell follows the user-drawn silhouette: a rounded horizontal capsule with a smooth centered dome, generated at the measured mobile width so the 56–60px avatar remains closely fitted without stretching the cap. No separate ring or arc surrounds the avatar. Profile suppresses the moving lens and may use only a soft avatar-derived halo. Labels and icons use near-black in light mode and white in dark mode. Reduced-motion preference removes lens travel, deformation, halo scale and rotating greeting transitions.
