...
The form language mixes precise controls with selectively softer instruments. Fields and compact controls use four- to ten-pixel corners. Grouped MODs and source instruments use 12–14px corners. Major sheets use 16–18px corners. The mobile navigation is a deliberate floating instrument with a 56px-tall moving pill lens; its 64px shell ends in exact 32px-radius semicircles, while the center avatar sits inside a 42px-radius circular rise joined to the shell by tangent rounded shoulders. The compact dispatch shortcut may use a capsule because it is a transient edge action, not a global control convention.
...
Desktop uses a sticky four-cell contents rail with consistent line icons. Mobile combines Kyant0's LiquidBottomTabs motion relationships with the same compact frosted material as the floating Build shortcut: the shell shares its low-alpha light/dark tint, 10px backdrop blur, saturation, rim and restrained shadow. Each work destination receives a 56px-tall pill lens that stays vertically centered inside the 64px shell at rest, with highlight, inner depth and restrained chromatic edge separation. Pressing grows and velocity-deforms the lens independently on each axis while the whole panel receives a damped four-pixel offset and a subtle 16px-relative scale. Release settles with exponential ease-out and no bounce. The shell follows the user-drawn silhouette: a horizontal capsule with exact semicircular ends whose center rise follows a 42px-radius circle around the 56–60px avatar. Short tangent Bézier shoulders merge the rise into the shell without pointed joins, and the path is generated at the measured mobile width without stretching into an ellipse. No separate ring or arc surrounds the avatar. Profile suppresses the moving lens and may use only a soft avatar-derived halo. Labels and icons use near-black in light mode and white in dark mode. Reduced-motion preference removes lens travel, deformation, halo scale and rotating greeting transitions.
